...
export async function GET(req) {
  ...
  try {
    ...
    if (getAllUsers === 'true') {
      ...
      // Fetch all users and manually filter sensitive fields
...
export async function POST(req) {
  try {
    ...
    if (action === 'update') {
      ...
      // Branch updates - admin only (Only admin can update branches)
      if (branch !== undefined) {
        try {
          requireRole(user, ['admin'])
        } catch (roleError) {
    ...
    const newUser = {
      ...
      name: body.name.trim(),
      phone: body.phone || '',
      role: isAdminCreation && body.role && ALLOWED_ROLES.includes(body.role)
            ? body.role
...
export async function PUT(req) {
  ...
  try {
    ...
    const existingUser = await db.collection('user').findOne({ email })
    if (!existingUser) {
      // If upload succeeded but user not found, clean up uploaded image
      try {
